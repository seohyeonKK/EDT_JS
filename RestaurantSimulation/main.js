...
document.getElementById("soondae-soup").addEventListener("click", () => getOrder(new Food(soondae)));
document.getElementById("hangover-soup").addEventListener("click", () => getOrder(new Food(hangover)));
...
async function findAvailable(staffs){
  return new Promise((resolve) => {
    const timerId = setInterval(()=>{
      const availableStaff = staffs.filter((staff) => staff.status === '대기중')
      if (availableStaff.length > 0){
        clearInterval(timerId);
        resolve(availableStaff[0]);
      }
    }, 100)
  });
}
...
function updateOrders() {
  ...
  pendingOrdersElement.innerHTML = orderList.map(order => `<li>주문${order.no} : ${order.name}</li>`).join('');
}
